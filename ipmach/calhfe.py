...
def OneStep_sTI(ti_windows, ti_window_steps, ti_sample_steps, minexe, exe,
    ...
    for i in range(1, ti_windows+1):
        ...
        print_sander_1step_mdf(str(i)+'_v1.in', 'ti', ti_windows_steps, lamada, ifc4)
...
def print_pmemd_2step_mdf(file_name, nxt, window_steps, lamada, fwdorbwd, force):
...
def print_sander_2step_mdf(file_name, nxt, window_steps, lamada, force, ifc4):
    ...
    if force == 'v':
        ...
        if ifc4 == 1:
            print print("  lj1264=1,", file=ti_mdf)
    print("/", file=ti_mdf)
    ti_mdf.close()
